...
Each of the following lines contains a value to be converted as well as its unit: m, g or c
...
Each conversion result should be printed on its own line,
...
The first is 10 meters, which, when converted, gives approximately 32.808 feet.
The second is 1245.243 grams, or about 2.745761 pounds,
the third is 37.2 degrees Celsius, or 98.96 degrees Farenheit,
and the fourth is 23 grams, or 0.050715 pounds.
...
double feet(double meter){
  double ft = meter * 3.2808;
  return ft;
}

double pounds(double gram){
  double g = gram * 0.002205;
  return g;
}

double fahrenheit(double celcius){
  double c = 32 + 1.8 * celcius;
  return c;
}
